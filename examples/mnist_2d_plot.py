...
# AutoCPCA with background
y_selection = (y == 2).astype(int)*2 + (y == 7).astype(int)*7

# comparison between both
auto_cpca = AutoCPCA(n_components=2, alpha={0: 7, 2: 0, 7: 0})
normal_pca = PCA(n_components=2)

# build fg and bgs only on train_set
a = StratifiedShuffleSplit(1, test_size=0.1)
train_index, test_index = next(a.split(X, y))
auto_cpca.fit(X[train_index], y_selection[train_index])
normal_pca.fit(X[train_index], y_selection[train_index])
...
plt.subplot(2, 2, 2)
plt.scatter(trans_normal_pca_X[:, 0], trans_normal_pca_X[:, 1], c=y_selection[test_index],
            cmap=plt.cm.coolwarm, alpha=0.6)
plt.title('PCA selected classes')
plt.subplot(2, 2, 3)
plt.scatter(trans_X[:, 0], trans_X[:, 1], c=y[test_index],
            cmap=plt.cm.coolwarm, alpha=0.6)
plt.title('AutoCPCA all classes')
plt.subplot(2, 2, 4)
plt.scatter(trans_X[:, 0], trans_X[:, 1], c=y_selection[test_index],
            cmap=plt.cm.coolwarm, alpha=0.6)
plt.title('AutoCPCA selected classes')
plt.tight_layout()
plt.show()
plt.close()
